...
RUBY_PATCHES=(
	"${PN}-2.4.2-secret-location.patch"
)
...
all_ruby_prepare() {
	...
	sed -i -E \
		...
		-e "s|[\# ]*(secret_file: ).*|\1\"${DATA_DIR}/.gitlab_shell_secret\"|" \
...
all_ruby_install() {
	# install lib
	insinto ${DEST_DIR}; doins -r lib CHANGELOG LICENSE README.md VERSION
	...
	diropts -m02770; keepdir ${DATA_DIR}/repositories
	diropts -m755; dodir ${LOGS_DIR}

	# GitLab stupidly expects that gitlab-shell is inside the home of git user...
